$(function(){
	...
	$('.temptopage').click(function(){
		...
		if( confirm('重要：导出后一定要编辑被导出模板的导航和页面链接，否则前端可能无法显示！！！') ) {
			Http('post','json','temptopage',postdata,function(data){
				...
				if(data.status == 200){
					setTimeout(function(){
						location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=my&do=temp&m=xsx_v3';
					},500);
				}
			},true);
		}
	...
	function ajaxUrl(op){
		return window.sysinfo.siteroot + 'web/index.php?c=site&a=entry&op='+op+'&do=ajax&m=xsx_v3';
	};
