...
myyapp.controller('ctr',['$scope',function($scope){
		...
		data : [
			{
				...
				actimg: window.sysinfo.siteroot+'/addons/xsx_v3/public/images/thank.png',
				url : '',
                type : 'url',
			},
			{
	...
	$scope.saveData = function(){
        ...
        $.ajax({
            type: 'POST',
            dataType : 'json',
            url: window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=savebar&do=ajax&m=xsx_v3',
            data: {data : $scope.items, tid : tempid },
            success: function(data){
                if(data.status == 200){
                    alert("已保存！");
					//location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=list&do=bar&m=xsx_v3';
            	}else{
                    alert("保存失败");
                }
            },
