...
myyapp.controller('ctr',['$scope',function($scope){
	...
	$scope.modules = [
		{
			...
			params : {
				...
				data : [
					{
						id : '00000001',
						img: window.sysinfo.siteroot+'/addons/xsx_v3/public/images/stemp.png',
						type : 'url',
					}
				]
			}
				...
				data : [
					{
						id : '00000001',
						title : '导航名称',
						img: window.sysinfo.siteroot+'/addons/xsx_v3/public/images/thank.png',
						type : 'url',
						url : ''
					},
					{
	...
	$scope.otherurl = [
		{title:'查看表单数据',url:'/xsx_v3/pages/form/form'},
		{title:'文章列表',url:'/xsx_v3/pages/artlist/artlist'}
	];
	...
	$scope.saveData = function(){
        ...
        $.ajax({
            type: 'POST',
            dataType : 'json',
            url: window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=savepage&do=ajax&m=xsx_v3',
            data: {data : items,basic : basic,tid : tempid,id : page && page.id ? page.id : 0 },
            success: function(data){
                if(data.status == 200){
                    alert("已保存！");
					if( op == 'add' ) location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=list&do=page&m=xsx_v3'+'&tid='+tempid;
            	}else{
                    alert("保存失败");
                }
            },
	...
	$scope.loadPageByid = function( id ){
		...
		if( confirm('此功能是将以前设计的页面导入，可进行编辑添加页面，确定导入页面吗？') ) {
			location.href = window.sysinfo.siteroot + '/web/index.php?c=site&a=entry&op=add&do=page&m=xsx_v3'+'&tid='+tempid + '&lid='+id ;
		}
