var goodsVue = new Vue({
    ...
    data: {
        ...
        goods_type: 0,//药品类型：1=团购(groupon)，2=抢购(rush)，3=拼团(wlfightgroup)，4=砍价(bargain)
        type_name: '团购',//当前药品类型名称
        infoData: [],//药品信息数组
        page:1,//当前页
        page_num:1,//总页数
        statusList:[],//状态列表
        classList:[],//分类列表
        getGoodsLink: '',//获取药品信息列表的请求地址
        copyGoodsLink: '',//复制药品请求地址
        setGoodsSale: '',//设置药品上架/下架请求地址
        toExamineLink: '',//药品审核请求地址
        deleteGoodsLink:'',//删除药品请求地址
    },
    methods: {
        //获取默认药品类型
        goodClass(){
        ...
        //推荐药品 目前仅团购药品由此功能
        goodsRecommend(id){
            tip.confirm( '确认将此药品设置为推荐药品？',function () {
        ...
        isSale(id,status){
            ...
            tip.confirm( '确认将此药品'+statushtml+'？',function () {
        ...
        goodsToExamine(id,flag){
            ...
            tip.confirm( '确认药品审核'+statushtml+'？',function () {
        ...
        //删除药品信息
        deleteGoodsInfo(id){
            tip.confirm( '此操作会删除当前药品，同时会导致订单药品数据缺失或其他问题，确定要删除吗？',function () {
                let res = commonVue.requestAjax(goodsVue.deleteGoodsLink,{id:id});
                goodsVue.goodsList();//重新获取信息列表
            });
        },
        //改变基本信息 药品类型：1=团购，2=抢购，3=拼团，4=砍价
        changeInfo() {
            //配置当前药品的基本操作信息
