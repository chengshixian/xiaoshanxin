...
common.appUrl = function(routes, params,complete = false){
    ...
    if(complete){
        let http = window.location.href.split("web");
        url = http[0]+'app/index.php?i=' + window.sysinfo.uniacid + '&c=entry&m=xsx_v3&p=' + strs[0] + '&ac=' + strs[1] + '&do=' + strs[2];
    }else{
        url = './index.php?i=' + window.sysinfo.uniacid + '&c=entry&m=xsx_v3&p=' + strs[0] + '&ac=' + strs[1] + '&do=' + strs[2];
    }
    if(params) {
        if(typeof(params) == 'object') {
            url += "&" + common.toQueryString(params)
